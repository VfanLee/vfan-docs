...
另一方面，你也可以使用 `Promise.reject()` 来明确地返回一个已拒绝状态的 Promise，并提供自定义的拒绝原因。这种方式允许你更精细地控制 Promise 的状态，而不仅仅依赖于错误抛出来触发拒绝。

```js
async function asyncFunction() {
  return Promise.reject("Custom rejection reason");
}

asyncFunction()
  .catch(error => {
    console.error(error); // 输出: "Custom rejection reason"
  });
```
