...
#  root 用户通过 SSH 登录
 ...
 如果 Ubuntu 是新安装的，你会发现 root 用户无法通过 SSH 登录。这是因为 root 权限太高可以做任何事，安全起见Ubuntu下的SSH默认不让root登录。

 方案1：
    假如一个普通用户有sudo权限，那就可以以普通用户先SSH登录，再切换至 root
 
 方案2：
    vi /etc/ssh/sshd_config
    找到 PermitRootLogin prohibit-password ，更改为 PermitRootLogin yes，重启 ssh 服务